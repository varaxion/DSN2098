...
from wtforms import Form, StringField, PasswordField, IntegerField, validators, EmailField
...
db = pymysql.connect(
    ...
    cursorclass=pymysql.cursors.DictCursor  # This will ensure the returned rows are in dictionary format 
    )
...
def reset_request():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if result == 0:
            ...
            cur.close()
            s = JWTSerializer(app.config['SECRET_KEY'])
